...
Create a support ticket with `APP-CERT` reference to get it installed on the Cloud instance _OR_ follow the cloud-ops steps to install non-published applications.
...
- Enter the following details in the pop up box:
  ...
  - **Datadog Site** (_required_): Enter the Datadog Site for this account. As of March 2026, the following Datadog sites are available:
    | Parameter  | Site URL |
    |------------|----------|
...
- Since version 2.0.0, the **Datadog Add-on for Splunk** has been rebuilt using the
  [UCC Framework](https://splunk.github.io/addonfactory-ucc-generator/) and tested on
  Splunk 10.2.0 (the latest version at the time of writing), using Datadog trial accounts.
- Although we have not tested it against all modern Splunk versions, we strongly believe
  it is compatible with Splunk 9.x and 10.x. If you run into any compatibility issues,
  please reach out!
